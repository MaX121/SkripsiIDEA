...
public class BufferOccupancyReport extends Report implements UpdateListener {
	...
	public void done() {
		...
		/* double temp = 0;
		for (Double bufferAvg : bufferCounts.values())  temp += bufferAvg;
		System.out.println(temp/bufferCounts.size());
		write("Total Average Buffer Occupancy : " + temp + " / " + bufferCounts.size() + " : " + (temp/bufferCounts.size())); */
		super.done();
	}
