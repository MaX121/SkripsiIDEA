...
public abstract class ActiveRouter extends MessageRouter {
	...
	public ActiveRouter(Settings s) {
		super(s);
		if (s.contains(DELETE_DELIVERED_S)) { this.deleteDelivered = s.getBoolean(DELETE_DELIVERED_S); }
		else { this.deleteDelivered = false; }
	}
	...
	public boolean requestDeliverableMessages(Connection con) {
		...
		for (Message m : temp) { if (other == m.getTo()) { if (startTransfer(m, con) == RCV_OK) { return true; } } }
		return false;
	}
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		if (recvCheck != RCV_OK) { return recvCheck; }
